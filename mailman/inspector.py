...
def submissions(assignment, username):
    ...
    if assignment:
        query = query.where(db.Submission.assignment == assignment)
    if username:
        query = query.where(db.Submission.user == username)
    for sub in query:
        print(sub.submission_id,
              datetime.fromtimestamp(sub.timestamp).isoformat(),
              sub.user, sub.assignment)


def missing(assignment):
    query = db.Submission.select().where(db.Submission.assignment == assignment)  # NOQA: E501
